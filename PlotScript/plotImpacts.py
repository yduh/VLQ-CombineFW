...
with open(args.input) as jsonfile:
    data = copy.deepcopy(json.load(jsonfile))

if skip_gt_in_plot:
    for index,item in enumerate(data['params']):
        if item['name'] == 'gt':
            #print "Removing gt from data['params']"
            data['params'].pop(index)
...
for page in xrange(n):
    #canv = ROOT.TCanvas(option.output, option.output)
    ...
    plot.DrawTitle(pads[1], '#hat{#it{Y}_{t}} = %.3g^{+ %.3g}_{- %.3g}' % (
        POI_fit[1], (POI_fit[2] - POI_fit[1]) ,
        (POI_fit[1] - POI_fit[0])), 3)
    extra = ''
    if page == 0:
        extra = '('
    if page == n - 1:
        extra = ')'
    canv.Print(args.output+'%s' % extra)
